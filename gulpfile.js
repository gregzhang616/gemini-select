...
var gulp = require('gulp'),
    ...
    componentSassList = [
      ...
      srcSassPath + 'select.scss',
      srcSassPath + 'dropdown.scss'
    ];
...
gulp.task('process-component-sass', function () {
  return gulp.src(componentSassList)
    ...
    .pipe($dev().concat('gemini.select.css'))
    .pipe(gulp.dest(distBasePath + 'css'))
    .pipe($dev().minifyCss())
    .pipe($dev().rename('gemini.select.min.css'))
    .pipe(gulp.dest(distBasePath + 'css'))
    .pipe($dev().connect.reload());
});
...
gulp.task('process-component-js', function () {
  return gulp.src(srcBasePath + 'js/*.js')
    .pipe($dev().concat('gemini.select.js'))
    .pipe(gulp.dest(distBasePath + 'js/'))
    .pipe(gulpUglify())
    .pipe($dev().rename('gemini.select.min.js'))
    .pipe(gulp.dest(distBasePath + 'js'))
    .pipe($dev().connect.reload());
});
